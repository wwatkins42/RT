...
static char	*get_full_name(t_env *e, char *name)
{
	...
	time(&epoch);
	!(buffer = ft_strdup(name)) ? error(e, E_MALLOC, NULL, 1) : 0;
	...
	ft_strdel(&buffer);
	del = f_name;
	!(f_name = ft_strjoin(f_name, ".yml")) ? error(e, E_MALLOC, NULL, 1) : 0;
	ft_strdel(&del);
	return (f_name);
}
...
void		yml_exporter(t_env *e, char *name)
{
	...
	f_name = get_full_name(e, name);
	if ((fd = open(f_name, FILE_ARG, FILE_RIGHTS)) == -1)
	{
		error(e, strerror(errno), f_name, 0);
		ft_strdel(&f_name);
		return ;
	}
	ft_strdel(&f_name);
	...
	export_object(fd, e);
	write(fd, "...\n", 4);
	close(fd) == -1 ? error(e, strerror(errno), NULL, 0) : 0;
}
